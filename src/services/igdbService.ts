...
async function getAccessToken(): Promise<string> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('Faltan VITE_TWITCH_CLIENT_ID o VITE_TWITCH_CLIENT_SECRET en .env.local')
  }
  ...
  tokenCache = { access_token: data.access_token, expires_at: Date.now() + (data.expires_in - 60) * 1000 }
...
async function translateToSpanish(text: string): Promise<string> {
  ...
  try {
    const url =
      ...
      new URLSearchParams({ client: 'gtx', sl: 'en', tl: 'es', dt: 't', q: text })

    const res  = await fetch(url)
    const data = (await res.json()) as string[][][]
    const translated = data.flat().map((c) => c?.[0] ?? '').join('')
...
export async function fetchIGDBGame(slug: string): Promise<IGDBGame> {
  // 1. Intenta desde cache precargado
  const cached = igdbCache[slug]
